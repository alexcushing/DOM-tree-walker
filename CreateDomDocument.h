...
private:
  /**
   * @name fUnicodeForm
   * @purpose This is the Unicode XMLCh format of the string.
   */
  XMLCh* fUnicodeForm;
};
...
public:
  /**
   * @name CreateDOMDocument Constructor
   * @purpose Creates a custom XML file in memory through Xerces DOMDocument library
   */
  CreateDOMDocument();

  /**
   * @name PrintXML
   * @param the previously created DOMDocument which was created in memory with developer's desired XML info
   */
  void PrintXML(DOMDocument* doc);
  /**
   * @name Tree Walker
   * @param rootElem DOMElement* to print DOmtree recursively
   * @param the previously created DOMDocument which was created in memory with developer's desired XML info
   */
  void TreeWalker( DOMElement* rootElem, DOMDocument* doc);
private:
